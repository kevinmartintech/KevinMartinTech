...
[RowUpdatePersonId] [int] NOT NULL CONSTRAINT [Person_RowUpdatePersonId_Default] DEFAULT ((-1)),
[RowUpdateTime] [datetime2] NOT NULL CONSTRAINT [Person_RowUpdateTime_Default] DEFAULT (sysdatetime()),
[RowCreateTime] [datetime2] NOT NULL CONSTRAINT [Person_RowCreateTime_Default] DEFAULT (sysdatetime())
) ON [PRIMARY]
GO
ALTER TABLE [Application].[Person] ADD CONSTRAINT [Person_PersonId] PRIMARY KEY CLUSTERED  ([PersonId]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [Person_RowUpdatePerson] ON [Application].[Person] ([RowUpdatePersonId]) ON [PRIMARY]
GO
ALTER TABLE [Application].[Person] ADD CONSTRAINT [Person_RowUpdatePerson] FOREIGN KEY ([RowUpdatePersonId]) REFERENCES [Application].[Person] ([PersonId])
GO
...
GO
EXEC sp_addextendedproperty N'MS_Description', N'Middle name or middle initial of the person.', 'SCHEMA', N'Application', 'TABLE', N'Person', 'COLUMN', N'MiddleName'
GO
