...
[RowUpdateTime] [datetimeoffset] NOT NULL CONSTRAINT [Application_CountryRegion_RowUpdateTime_Default] DEFAULT (sysdatetimeoffset()),
[RowCreateTime] [datetimeoffset] NOT NULL CONSTRAINT [Application_CountryRegion_RowCreateTime_Default] DEFAULT (sysdatetimeoffset())
) ON [PRIMARY]
GO
ALTER TABLE [Application].[CountryRegion] ADD CONSTRAINT [Application_CountryRegion_CountryRegionId] PRIMARY KEY CLUSTERED  ([CountryRegionId]) ON [PRIMARY]
GO
ALTER TABLE [Application].[CountryRegion] ADD CONSTRAINT [Application_CountryRegion_CountryName] UNIQUE NONCLUSTERED  ([CountryName]) ON [PRIMARY]
GO
ALTER TABLE [Application].[CountryRegion] ADD CONSTRAINT [Application_CountryRegion_FormalName] UNIQUE NONCLUSTERED  ([FormalName]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [Application_CountryRegion_RowUpdatePersonId] ON [Application].[CountryRegion] ([RowUpdatePersonId]) ON [PRIMARY]
GO
ALTER TABLE [Application].[CountryRegion] ADD CONSTRAINT [Application_CountryRegion_Application_RowUpdatePerson] FOREIGN KEY ([RowUpdatePersonId]) REFERENCES [Application].[Person] ([PersonId])
GO
EXEC sp_addextendedproperty N'MS_Description', N'Country or Region that contain the states or provinces (including geographic boundaries)', 'SCHEMA', N'Application', 'TABLE', N'CountryRegion', NULL, NULL
GO
